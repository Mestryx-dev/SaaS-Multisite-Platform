...
Platform console layout patterns (Wave A+): `PageContent`, `TableFrame`, `SplitLayout`, `FilterChips`, `DensityToggle`, `BulkActionBar` — see [admin-console-master-plan.md](./admin-console-master-plan.md) and Storybook **Patterns/ConsoleLayout**. **Layout recipes R1–R8** (Wave G): [admin-console-layout-recipes.md](./admin-console-layout-recipes.md) — same operating logic within a family; Shipping is R1 SoT. UX polish + Stalwart harvest (2026-07-22): EmptyState icon/CTA, ListPanel, SearchField, ActionTile, Alert warning, RouteFade. **Dokploy cleanliness** (2026-07-22): flat AppShell nav, ListPanel list pages, surface `/60`, Button `inverse`. **Fluid width** (2026-07-24): `PageContent` `wide`/`full` fill the AppShell main column (no `max-w-7xl`); `SplitLayout` `listDetail` for Sites; `FormPanel` default `xl`. **Unified header** (2026-07-25): AppShell full-bleed sticky bar (`--app-header-height`); sidebar below; org switcher centered (`topBarCenter`); utilities + theme toggle on the right.
